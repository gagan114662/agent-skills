...
6. **Additive `0004_tasks` migration with a clean down; `tasks` graduates out of the stub file.** (Numbered after #7's `0003_search`, which merged to main first.) The migration adds `description`/`labels`/`updated_at` + the status `CHECK` to `tasks` and creates the three new tables; the down reverses all of it (CI down/up step green). In the Drizzle schema `tasks` moves from `schema/stubs.ts` into its own `schema/tasks.ts`; `memories`/`memory_edges`/`permissions` stay stubs.

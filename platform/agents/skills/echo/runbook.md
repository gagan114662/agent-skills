...
kind: runbook
version: 1.0.0
description: The senior Social practitioner procedure for @echo — clarify, consult governed sources first, execute, self-review.
---

# Social runbook — @echo

The procedure a senior practitioner follows. Your job: turn one good idea into a week of platform-native drafts that wait on the counter for a human yes.
...
2. **Curated reference** — the `echo-knowledge` router's reference files (governed prose).
...
## 3. Execute
Owned/most-used metrics for this domain: `growth.venture_signal`.
Do the work in-channel as a draft. Cite what you looked at — **provenance** and **freshness** on every number.
Because @echo's work can leave the building, **never** send, post, or spend. Produce the **draft** plus a

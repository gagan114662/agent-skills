/**
 * A tiny, dependency-free frontmatter parser for the prerendered blog (#252). It is a faithful copy of
 * the server's marketing-site parser (`apps/server/src/site/frontmatter.ts`, #153) — the two are kept
 * deliberately separate because the blog is a *build-time* surface bundled into the web app (it runs in
 * the browser and in the Vite SSR prerender), whereas the server copy runs in the API process. Pulling
 * in `gray-matter` (+ its `js-yaml` transitive) for a handful of flat scalars would violate the web
 * app's "no heavy deps" budget (#149), so this supports exactly the subset blog frontmatter needs:
 * top-level `key: value` scalars and `key:` / indented `- item` string sequences. Pure and total.
 */
